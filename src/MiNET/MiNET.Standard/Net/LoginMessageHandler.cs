...
namespace MiNET
{
	public class LoginMessageHandler : IMcpeMessageHandler
	{
		...
		protected void DecodeCertMono(McpeLogin message)
		{
			...
			try
			{
				JwtBase64UrlEncoder a = new JwtBase64UrlEncoder();
				{

					JwtParts parts = new JwtParts(skinData);
					
					var headerJson = Encoding.UTF8.GetString(a.Decode(parts.Header));
					IDictionary<string, dynamic> headers = JsonConvert.DeserializeObject<Dictionary<string, object>>(headerJson);

					dynamic payload = JObject.Parse(Encoding.UTF8.GetString(a.Decode(parts.Payload)));
				...
				//if (!isMono)
				{
					...
					{
						JwtParts parts = new JwtParts(token.ToString());
						IDictionary<string, dynamic>
							headers = JsonConvert.DeserializeObject<Dictionary<string, object>>(Encoding.UTF8.GetString(a.Decode(parts.Header)));
						...
						if (Log.IsDebugEnabled)
						{
							...
							dynamic jsonPayload = JObject.Parse(Encoding.UTF8.GetString(a.Decode(parts.Payload)));
				...
				if (useEncryption)
				{
					...
					/*	RijndaelManaged rijAlg = new RijndaelManaged
						...
						var para = new ParametersWithIV(new KeyParameter(secret), secret.Take(16).ToArray());
						var cipherEncrypt = CipherUtilities.GetCipher("AES/CFB8/NoPadding");
						cipherEncrypt.Init(true, para);

						var cipherDecrypt = CipherUtilities.GetCipher("AES/CFB8/NoPadding");
						cipherDecrypt.Init(false, para);

						// Create a decrytor to perform the stream transform.
						//ICryptoTransform decryptor = rijAlg.CreateDecryptor(rijAlg.Key, rijAlg.IV);
						MemoryStream inputStream = new MemoryStream();
						CipherStream cryptoStreamIn = new CipherStream(inputStream, cipherDecrypt, cipherEncrypt);
						...
						CipherStream cryptoStreamOut = new CipherStream(outputStream, cipherDecrypt, cipherEncrypt);
						...
						string val = CryptoUtils.Encode(handshakeJson, keyPair.Private,
